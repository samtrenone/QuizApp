function toggleIndex() {
  ...
  controlElements.showIndex.textContent =
    quiz.elements.index.classList.contains("index--show")
      ? "Hide index"
      : "Show index";
}
...
function filterQuestionsIndex(e) {
  if (e.target.matches("input")) {
    let questions = Array.from(
      document.getElementsByClassName("index__question")
    );
    ...
    switch (e.target.value) {
      ...
      case "unanswered":
        questions
          .filter((el) => el.dataset.answered === "true")
          .forEach((el) => el.classList.add("hide"));
        break;
    }
  }
...
function renderIndex() {
  ...
  quiz.questions.forEach((question, index) => {
    ...
    newLi.innerHTML = `<i class="fa fa-bookmark mark"></i><p class="index__question-index">${
      index + 1
    }</p>${dataQuestion.question}`;
...
function updateAnsweredClassesIndex(id) {
  ...
  if (quiz.showResults) {
    if (quizQuestion.result == "right")
      questionEl.classList.add("index__question--right");
    if (quizQuestion.result == "wrong")
      questionEl.classList.add("index__question--wrong");
  }
}
...
function updateIndexCurrent() {
  ...
  let currentQuestionEl = quiz.elements.index.querySelector(
    `[data-index-id="${id}"]`
  );

  let currentQuestionBeforeEl = quiz.elements.index.querySelector(
    ".index__question--current"
  );
  if (currentQuestionBeforeEl)
    currentQuestionBeforeEl.classList.remove("index__question--current");
