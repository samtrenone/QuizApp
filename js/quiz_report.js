function renderQuizReport() {
  ...
  for (let i = 0; i < dataDomains.length; i++) {
    quizReportContent += `<p>
        <span class="quiz__report-answered">${
          dataDomains[i].title
        }: ${quiz.getTotalAnsweredByDomain(
      i
    )} of ${quiz.getTotalQuestionsByDomain(i)}</span>
        <span class="quiz__report-right">${quiz.getTotalRightByDomain(
          i
        )} / ${quiz.getPercentageRightByDomain(i)}%</span>
        <span class="quiz__report-wrong">${quiz.getTotalWrongByDomain(
          i
        )} / ${quiz.getPercentageWrongByDomain(i)}%</span>
    </p>`;
  }
...
function filterQuestionsReport(e) {
  if (e.target.matches("input")) {
    let questions = Array.from(
      document.getElementsByClassName("quiz__report-question")
    );
    ...
    switch (e.target.value) {
      case "marked":
        questions
          .filter((el) => el.dataset.marked !== "true")
          .forEach((el) => el.classList.add("hide"));
        break;
      case "right":
        questions
          .filter((el) => el.dataset.result !== "right")
          .forEach((el) => el.classList.add("hide"));
        break;
      case "wrong":
        questions
          .filter((el) => el.dataset.result !== "wrong")
          .forEach((el) => el.classList.add("hide"));
        break;
    }
  }
...
function renderQuizReportQuestions() {
  ...
  quiz.questions.forEach((question, index) => {
    ...
    newLi.addEventListener("click", function () {
      this.querySelector(".quiz__report-panel").classList.toggle(
    ...
    question.answers.forEach((answer) => {
      ...
      newLiAnswer.textContent = dataAnswer.answer;
      newUlAnswers.appendChild(newLiAnswer);
    });

    newLi.innerHTML = `<i class="fa fa-bookmark mark"></i><p class="quiz__report-question-index">${
      index + 1
    }</p>
        ...
        <p class="quiz__report-reference">${dataQuestion.domain}<br/>${
      dataQuestion.source
    }</p>
        </div>`;
